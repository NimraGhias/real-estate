  ...
  return (
    <section id="services" className="py-24 lg:py-32 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-br from-amber-50/30 via-transparent to-blue-50/30 rounded-full blur-3xl" />
      </div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div ref={ref} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {services.map((service, i) => (
            <div
              key={service.id}
              className={`group relative p-[1px] rounded-2xl overflow-hidden transition-all duration-700 hover:shadow-2xl hover:shadow-gray-900/10 hover:-translate-y-1 ${
                inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
              }`}
              style={{ transitionDelay: `${i * 100}ms` }}
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${service.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
              <div className="relative p-6 lg:p-8 bg-white rounded-2xl h-full group-hover:bg-white/95 transition-colors duration-500 flex flex-col">
                <div className="absolute top-4 right-4 text-5xl lg:text-6xl font-bold text-gray-100 select-none leading-none">
                  {String(service.id).padStart(2, '0')}
                </div>

                <div className={`w-14 h-14 ${service.lightBg} ${service.lightText} rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:shadow-lg transition-all duration-300 relative shrink-0`}>
                  <div className={`absolute inset-0 ${service.lightBg} blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
                ...
                <ul className="mt-4 space-y-2">
                  {service.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-sm text-gray-400 group-hover:text-gray-500 transition-colors">
                      <div className={`w-1.5 h-1.5 rounded-full ${service.lightText} shrink-0`} />
                      {f}
                    </li>
                  ))}
                </ul>

                <div className="mt-auto pt-5 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-all duration-300 -translate-x-2 group-hover:translate-x-0">
                  <span className={`text-sm font-semibold ${service.lightText} flex items-center gap-1.5`}>
                    Learn more
